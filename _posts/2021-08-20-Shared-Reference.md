...
Our _copy constructor_ is not technically a copy-constructor because at the time we pass `shared_reference<T>&` we only want it to **share** the contents of `m_ptr` and increment `m_counter`. What we want is to provide an interface for _sharing_ references. To avoid confusion, I will refer to `shared_reference(shared_reference<T>&)` as a reference constructor.
